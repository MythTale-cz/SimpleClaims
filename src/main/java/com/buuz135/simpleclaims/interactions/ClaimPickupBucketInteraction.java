...
public class ClaimPickupBucketInteraction extends RefillContainerInteraction {
    ...
    protected void firstRun(@NonNullDecl InteractionType type, @NonNullDecl InteractionContext context, @NonNullDecl CooldownHandler cooldownHandler) {
        ...
        if (targetBlock == null && playerRef != null) {
            Vector3d playerLoc = playerRef.getTransform().getPosition();
            if (ClaimManager.getInstance().isAllowedToInteract(playerRef.getUuid(), player.getWorld().getName(), (int) playerLoc.x, (int) playerLoc.z, defaultInteract)) {
